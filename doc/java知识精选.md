...
9. RabbitMq的可靠性投递
	1. 在4个环节实现可靠性. 
		...
		3. 消息在Queue中存储
			...
			2. 队列持久化
			3. 消息持久化
		4. 消费者订阅并消费消息: 消费者开启消息确认机制: basic.ack, basic.nack, basic.reject
		
	2. 消费者回调: 消费者处理完消息后, 回调生产者的api, 通知消息处理完毕.
	3. 补偿机制: 对于一定时间内没有响应的消息, 设置定时重发机制.
	4. 消息幂等性: 生产者: messageId和对每条消息生成唯一一个业务id, 消费者: 通过日志或者建表来做重复控制
	5. 消息的顺序性: 消费者消费的顺序和生产者生产的顺序一致.
1、消息队列的作用与使用场景？
    * 异步: 批量数据异步处理. 例: 批量上传文件
...
3、多个消费者监听一个生产者时，消息如何分发？
    ...
    * fair dispatch(公平分发)
4、无法被路由的消息，去了哪里？
    * 无法被路由的消息, 到死信队列或者丢失,处理这种情况有两种方式
    1. 使用mandatory=true配合ReturnListener, 实现消息回发
    2. 声明交换机时, 指定备份交换机
5、消息在什么时候会变成Dead Letter（死信）？
    - 在队列中的消息无法被消费, 会到死信交换机, 到死信交换机的消息被称为死信
    * 队列中的消息过了有效期
    * 队列中的消息没有被消费者消费接受, 消费者拒绝或队列拒绝消息重新入队
    * 队列中消息发生消息堆积
6、RabbitMQ如何实现延迟队列？
    * 开启插件
    * 消息过期时间+死信交换机(DLX_EXCHANGE)+死信队列(DLX_QUEUE)的特性
8、如何在服务端和消费端做限流？
    1. 服务端流控, 内存和磁盘配置限制
    2. 消费端流控: prefetch count参数控制
    3. 网关或接入层: 把消息拦截在请求之前
9、如何保证消息的顺序性？
    * 无法保证, 除非一个生产者一个消费者
    * 全局id, messageId, parentMsgId. 控制如果消息没有应答则不发送吓一跳消息
10、RabbitMQ的集群模式和集群节点类型？
    1. 普通模式
    2. 镜像模式.
    3. 节点类型: 磁盘节点(消息保存在磁盘和内存里面), 内存节点(消息没有持久化, 只保存在内存)
